...
global equity, emerging market equity, global bonds, UK gilts, gold) and combines
them into three portfolios (Cautious, Balanced,Adventurous). It then backtets each one, rebalances them once a year, and works out where the returns came from. The output is a one-page summary for each portfolio.
